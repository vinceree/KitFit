"use strict";(()=>{var A=Object.defineProperty;var O=(g,f,m)=>f in g?A(g,f,{enumerable:!0,configurable:!0,writable:!0,value:m}):g[f]=m;var u=(g,f,m)=>O(g,typeof f!="symbol"?f+"":f,m);(function(){let g={apiUrl:"https://kitfit.app",buttonText:"Try it on your bike \u2192"},f="kf-pending-job",m="kf-completed-job",F=3e3,w=300*1e3,H=`
    :host {
      all: initial;
  ...
  `,j=[{id:"alpine",name:"Alpine",desc:"Mountain road, sunny"},{id:"coastal",name:"Coastal",desc:"Ocean road, bright"},{id:"forest",name:"Forest",desc:"Tree-lined, dappled light"},{id:"urban",name:"Urban",desc:"City streets, dawn"}],E="kf-person-photos",S="kf-bike-photos",q=3;function L(a){try{return JSON.parse(sessionStorage.getItem(a)||"[]")}catch{return[]}}function I(a,e){let t=L(a).filter(o=>o.name!==e.name);t.unshift(e),t.length>q&&t.pop();try{sessionStorage.setItem(a,JSON.stringify(t))}catch{t.pop();try{sessionStorage.setItem(a,JSON.stringify(t))}catch{}}}function T(a,e=1600,t=.85){return new Promise((o,n)=>{let i=new Image;i.onload=()=>{let{width:s,height:r}=i;(s>e||r>e)&&(s>r?(r=r/s*e,s=e):(s=s/r*e,r=e));let d=document.createElement("canvas");d.width=s,d.height=r,d.getContext("2d").drawImage(i,0,0,s,r),URL.revokeObjectURL(i.src),o(d.toDataURL("image/jpeg",t))},i.onerror=()=>{URL.revokeObjectURL(i.src),n(new Error("Failed to load image for compression"))},i.src=URL.createObjectURL(a)})}function $(a,e){let[t,o]=a.split(","),n=t.match(/:(.*?);/)?.[1]||"image/jpeg",i=atob(o),s=new Uint8Array(i.length);for(let r=0;r<i.length;r++)s[r]=i.charCodeAt(r);return new File([s],e,{type:n})}function J(){if(document.getElementById("kf-toast-styles"))return;let a=document.createElement("style");a.id="kf-toast-styles",a.textContent=`
    ...
    `,document.head.appendChild(a)}function b(a,e){v(),J();let t=document.createElement("div");t.id="kf-toast-notification",t.innerHTML=`
    ...
    `,t.addEventListener("click",o=>{if(o.target.closest(".kf-toast-close")){v();return}v(),e?e():a.pageUrl&&a.pageUrl!==window.location.href&&(window.location.href=a.pageUrl)}),document.body.appendChild(t),setTimeout(()=>v(),3e4)}function v(){let a=document.getElementById("kf-toast-notification");a&&(a.style.animation="kf-toast-out 0.3s ease-in forwards",setTimeout(()=>a.remove(),300))}let h=null;function y(){h?.abort(),h=null}function C(a){y(),h=new AbortController,R(a,h.signal)}function P(){if(h&&!h.signal.aborted)return;let a=localStorage.getItem(f);if(!a)return;let e;try{e=JSON.parse(a)}catch{localStorage.removeItem(f);return}if(Date.now()-e.startedAt>w){localStorage.removeItem(f);return}C(e)}async function R(a,e){let{jobId:t,apiUrl:o}=a;for(;!e.aborted;){try{let n=await fetch(`${o}/api/tryon/jobs/${t}`,{signal:e});if(!n.ok)break;let i=await n.json();if(i.status==="completed"&&i.resultImage){let s={jobId:t,resultImage:i.resultImage,pageUrl:a.pageUrl,completedAt:Date.now()};localStorage.setItem(m,JSON.stringify(s)),localStorage.removeItem(f),document.dispatchEvent(new CustomEvent("kf-job-complete",{detail:s}));return}if(i.status==="failed"){localStorage.removeItem(f),document.dispatchEvent(new CustomEvent("kf-job-error",{detail:{jobId:t,error:i.error||"Generation failed"}}));return}if(Date.now()-a.startedAt>w){localStorage.removeItem(f);return}}catch{if(e.aborted)return}await new Promise(n=>setTimeout(n,F))}}class U{constructor(e,t){u(this,"config");u(this,"root");u(this,"personFile",null);u(this,"bikeFile",null);u(this,"selectedScene","alpine");u(this,"complements",[]);u(this,"selectedComplement",null);u(this,"activeJobId",null);u(this,"lastResultImage",null);u(this,"modalOpen",!1);u(this,"isMobile",!1);this.config={...g,...t},this.config.apiUrl.endsWith("/")&&(this.config.apiUrl=this.config.apiUrl.slice(0,-1)),this.root=e.attachShadow({mode:"open"}),this.render(),this.listenForJobEvents(),this.checkForCompletedResult()}listenForJobEvents(){document.addEventListener("kf-job-complete",(e=>{let t=e.detail;this.onJobComplete(t)})),document.addEventListener("kf-job-error",(e=>{let{jobId:t,error:o}=e.detail;this.activeJobId===t&&this.onJobError(o)}))}checkForCompletedResult(){let e=localStorage.getItem(m);if(!e)return;let t;try{t=JSON.parse(e)}catch{localStorage.removeItem(m);return}t.pageUrl===window.location.href?(localStorage.removeItem(m),this.open(),this.showResult(t.resultImage)):b(t)}onJobComplete(e){x=!0,this.lastResultImage=e.resultImage,this.modalOpen&&this.activeJobId===e.jobId?(this.showResult(e.resultImage),localStorage.removeItem(m)):e.pageUrl===window.location.href?b(e,()=>{localStorage.removeItem(m),this.open(),this.showResult(e.resultImage)}):b(e),this.activeJobId=null}onJobError(e){this.activeJobId=null;let t=o=>this.root.querySelector(o);if(this.modalOpen){t("#kf-loading").style.display="none",t("#kf-form").style.display="block";let o=t("#kf-error");o.textContent=e,o.style.display="block"}}showResult(e){let t=o=>this.root.querySelector(o);t("#kf-loading").style.display="none",t("#kf-form").style.display="none",t("#kf-result").style.display="block",this.root.querySelector("#kf-result-img").src=e}render(){let e=document.createElement("style");e.textContent=H,this.root.appendChild(e),this.isMobile="ontouchstart"in window||navigator.maxTouchPoints>0;let t=document.createElement("button");t.className="kf-trigger",t.innerHTML=`
      ...
      `).join(""),t.querySelectorAll(".kf-complement-card").forEach(o=>{o.addEventListener("click",()=>{let n=o.dataset.complementId,i=this.complements.find(s=>s.id===n);i&&(this.selectedComplement?.id===i.id?this.selectedComplement=null:this.selectedComplement=i,this.renderComplements())})})}}async fetchImageFromUrl(e,t){try{let n=await(await fetch(e)).blob();return new File([n],t,{type:n.type})}catch{return null}}open(){if(this.root.querySelector(".kf-overlay").classList.add("open"),this.modalOpen=!0,this.lastResultImage){this.showResult(this.lastResultImage),localStorage.removeItem(m);return}this.complements.length===0&&this.fetchComplements()}close(){this.root.querySelector(".kf-overlay").classList.remove("open"),this.modalOpen=!1}}let x=!1;document.addEventListener("kf-job-complete",(a=>{x=!1,queueMicrotask(()=>{if(!x){let e=a.detail;b(e)}})}));function M(){document.querySelectorAll("[data-kitfit]").forEach(a=>{let e=a;if(e.shadowRoot)return;let t={apiKey:e.dataset.kitfitKey||"",apiUrl:e.dataset.kitfitApi||g.apiUrl,productId:e.dataset.kitfitProduct||void 0,garmentImageUrl:e.dataset.kitfitGarment||void 0,buttonText:e.dataset.kitfitButton||g.buttonText};new U(e,t)}),P()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",M):M(),window.KitFitWidget=U})();})();
