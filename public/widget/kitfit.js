"use strict";(()=>{var A=Object.defineProperty;var O=(f,d,c)=>d in f?A(f,d,{enumerable:!0,configurable:!0,writable:!0,value:c}):f[d]=c;var p=(f,d,c)=>O(f,typeof d!="symbol"?d+"":d,c);(function(){let f={apiUrl:"https://kitfit.app",buttonText:"Try it on your bike \u2192"},d="kf-pending-job",c="kf-completed-job",C=3e3,y=300*1e3,F=`
    :host {
      all: initial;
  ...
  `,M=[{id:"alpine",name:"Alpine",desc:"Mountain road, sunny"},{id:"coastal",name:"Coastal",desc:"Ocean road, bright"},{id:"forest",name:"Forest",desc:"Tree-lined, dappled light"},{id:"urban",name:"Urban",desc:"City streets, dawn"}],x="kf-person-photos",w="kf-bike-photos",j=3;function E(r){try{return JSON.parse(sessionStorage.getItem(r)||"[]")}catch{return[]}}function S(r,e){let t=E(r).filter(o=>o.name!==e.name);t.unshift(e),t.length>j&&t.pop();try{sessionStorage.setItem(r,JSON.stringify(t))}catch{t.pop();try{sessionStorage.setItem(r,JSON.stringify(t))}catch{}}}function L(r,e=1600,t=.85){return new Promise((o,i)=>{let n=new Image;n.onload=()=>{let{width:a,height:s}=n;(a>e||s>e)&&(a>s?(s=s/a*e,a=e):(a=a/s*e,s=e));let l=document.createElement("canvas");l.width=a,l.height=s,l.getContext("2d").drawImage(n,0,0,a,s),URL.revokeObjectURL(n.src),o(l.toDataURL("image/jpeg",t))},n.onerror=()=>{URL.revokeObjectURL(n.src),i(new Error("Failed to load image for compression"))},n.src=URL.createObjectURL(r)})}function J(r,e){let[t,o]=r.split(","),i=t.match(/:(.*?);/)?.[1]||"image/jpeg",n=atob(o),a=new Uint8Array(n.length);for(let s=0;s<n.length;s++)a[s]=n.charCodeAt(s);return new File([a],e,{type:i})}function R(){if(document.getElementById("kf-toast-styles"))return;let r=document.createElement("style");r.id="kf-toast-styles",r.textContent=`
    ...
    `,document.head.appendChild(r)}function h(r,e){k(),R();let t=document.createElement("div");t.id="kf-toast-notification",t.innerHTML=`
    ...
    `,t.addEventListener("click",o=>{if(o.target.closest(".kf-toast-close")){k();return}k(),e?e():r.pageUrl&&r.pageUrl!==window.location.href&&(window.location.href=r.pageUrl)}),document.body.appendChild(t),setTimeout(()=>k(),3e4)}function k(){let r=document.getElementById("kf-toast-notification");r&&(r.style.animation="kf-toast-out 0.3s ease-in forwards",setTimeout(()=>r.remove(),300))}let u=null;function b(){u?.abort(),u=null}function I(r){b(),u=new AbortController,q(r,u.signal)}function H(){if(u&&!u.signal.aborted)return;let r=localStorage.getItem(d);if(!r)return;let e;try{e=JSON.parse(r)}catch{localStorage.removeItem(d);return}if(Date.now()-e.startedAt>y){localStorage.removeItem(d);return}I(e)}async function q(r,e){let{jobId:t,apiUrl:o}=r;for(;!e.aborted;){try{let i=await fetch(`${o}/api/tryon/jobs/${t}`,{signal:e});if(!i.ok)break;let n=await i.json();if(n.status==="completed"&&n.resultImage){let a={jobId:t,resultImage:n.resultImage,pageUrl:r.pageUrl,completedAt:Date.now()};localStorage.setItem(c,JSON.stringify(a)),localStorage.removeItem(d),document.dispatchEvent(new CustomEvent("kf-job-complete",{detail:a}));return}if(n.status==="failed"){localStorage.removeItem(d),document.dispatchEvent(new CustomEvent("kf-job-error",{detail:{jobId:t,error:n.error||"Generation failed"}}));return}if(Date.now()-r.startedAt>y){localStorage.removeItem(d);return}}catch{if(e.aborted)return}await new Promise(i=>setTimeout(i,C))}}class T{constructor(e,t){p(this,"config");p(this,"root");p(this,"personFile",null);p(this,"bikeFile",null);p(this,"selectedScene","alpine");p(this,"complements",[]);p(this,"selectedComplement",null);p(this,"activeJobId",null);p(this,"lastResultImage",null);p(this,"modalOpen",!1);this.config={...f,...t},this.config.apiUrl.endsWith("/")&&(this.config.apiUrl=this.config.apiUrl.slice(0,-1)),this.root=e.attachShadow({mode:"open"}),this.render(),this.listenForJobEvents(),this.checkForCompletedResult()}listenForJobEvents(){document.addEventListener("kf-job-complete",(e=>{let t=e.detail;this.onJobComplete(t)})),document.addEventListener("kf-job-error",(e=>{let{jobId:t,error:o}=e.detail;this.activeJobId===t&&this.onJobError(o)}))}checkForCompletedResult(){let e=localStorage.getItem(c);if(!e)return;let t;try{t=JSON.parse(e)}catch{localStorage.removeItem(c);return}t.pageUrl===window.location.href?(localStorage.removeItem(c),this.open(),this.showResult(t.resultImage)):h(t)}onJobComplete(e){v=!0,this.lastResultImage=e.resultImage,this.modalOpen&&this.activeJobId===e.jobId?(this.showResult(e.resultImage),localStorage.removeItem(c)):e.pageUrl===window.location.href?h(e,()=>{localStorage.removeItem(c),this.open(),this.showResult(e.resultImage)}):h(e),this.activeJobId=null}onJobError(e){this.activeJobId=null;let t=o=>this.root.querySelector(o);if(this.modalOpen){t("#kf-loading").style.display="none",t("#kf-form").style.display="block";let o=t("#kf-error");o.textContent=e,o.style.display="block"}}showResult(e){let t=o=>this.root.querySelector(o);t("#kf-loading").style.display="none",t("#kf-form").style.display="none",t("#kf-result").style.display="block",this.root.querySelector("#kf-result-img").src=e}render(){let e=document.createElement("style");e.textContent=F,this.root.appendChild(e);let t=document.createElement("button");t.className="kf-trigger",t.innerHTML=`
      ...
      `).join(""),t.querySelectorAll(".kf-complement-card").forEach(o=>{o.addEventListener("click",()=>{let i=o.dataset.complementId,n=this.complements.find(a=>a.id===i);n&&(this.selectedComplement?.id===n.id?this.selectedComplement=null:this.selectedComplement=n,this.renderComplements())})})}}async fetchImageFromUrl(e,t){try{let i=await(await fetch(e)).blob();return new File([i],t,{type:i.type})}catch{return null}}open(){if(this.root.querySelector(".kf-overlay").classList.add("open"),this.modalOpen=!0,this.lastResultImage){this.showResult(this.lastResultImage),localStorage.removeItem(c);return}this.complements.length===0&&this.fetchComplements()}close(){this.root.querySelector(".kf-overlay").classList.remove("open"),this.modalOpen=!1}}let v=!1;document.addEventListener("kf-job-complete",(r=>{v=!1,queueMicrotask(()=>{if(!v){let e=r.detail;h(e)}})}));function U(){document.querySelectorAll("[data-kitfit]").forEach(r=>{let e=r;if(e.shadowRoot)return;let t={apiKey:e.dataset.kitfitKey||"",apiUrl:e.dataset.kitfitApi||f.apiUrl,productId:e.dataset.kitfitProduct||void 0,garmentImageUrl:e.dataset.kitfitGarment||void 0,buttonText:e.dataset.kitfitButton||f.buttonText};new T(e,t)}),H()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",U):U(),window.KitFitWidget=T})();})();
